...
results~\cite{PatrascuTh06,PatrascuTh07} that structures \emph{\` a la} van Emde Boas
(e.g., y-fast tries~\cite{Willard83}) using time $O(\log w)$ are optimal among those
using linear space. The lower bound proved in~\cite{PatrascuTh06,PatrascuTh07} has
actually several cases, another one is realised, for instance, in
exponential trees~\cite{AnderssonTh07}. A very comprehensive discussion 
...
\begin{enumerate}
  ...
  of~\cite{BoseDoDuHoMo13}, which was obtained using space $O(n w \log
  ...
  trees}~\cite{BilleLaRaSaSaWe15}, answering predecessor queries in time\footnote{The
  bound in~\cite{BilleLaRaSaSaWe15} is $O(w-\log(x^+-x^-))$. Our proofs are correct
  ...
  search}~\cite{DemaineJoPa04}, answering
  ...
  in~\cite{AnderssonTh07}, and improves exponentially over
  interval-biased search trees, which need time  $O(\log(2^w-y)-\log
  D(x,s))$~\cite{BilleLaRaSaSaWe15}.
\end{enumerate}
...
\emph{fat binary search in a z-fast trie}~\cite{BelazzouguiBoPaVi09} in which the initial search interval
