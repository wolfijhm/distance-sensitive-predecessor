...
we can use fat binary search to quickly find the 
(length of) the name of the exit node $\exit(q)$ of $q$. 
Now, we explain how we can use this
information to find the predecessor of $q$ in $S$.
Given $q$ and the length $t$ of the name of $\exit(q)$, 
we denote by  $\pred(q, t)$ the index of the predecessor of 
$q$ in the lexicographic order of $S$.
...
\end{lemma}
\begin{proof}
We check if $q \preceq \extent(q[0, t - 1])$, or if the first bit of 
