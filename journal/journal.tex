...
We will use binary logarithms throughout, and we set 
$\log x := 1$ for $x \leq 2$.
...
\end{align*}
If the maximum or minimum does not exists, we set $q^- = ¸\bot$ or 
...
extents along the corresponding compressed paths. In particular, 
...
\caption{(above) A compacted trie, the 
