...
bounded universe of $w$-bit integers.  
In a \emph{predecessor (successor) search} structure,
our task is to store a set $S \subseteq U$, such 
that for any given $q \in U$, we can find the
largest element $q^- \in S$ smaller than $q$ (respectively, the smallest 
element $q^+ \in S$ larger than or equal to $q$).
In this paper, we consider both the static setting
(i.e., $S$ is fixed and does not change over time)
and the dynamic setting
(i.e., $S$ can change through insertions and deletions).

In the static setting, we
assume 
that the $n$ elements of $S$ are available for inspection, and we 
present a number of algorithms that, with a small additional 
index (usually of $O(n\log w)$ bits),
can answer predecessor/successor queries quickly and 
with time bounds that depend on different kinds of \emph{distance}
between the query element and $S$,
improving significantly several results that appeared in the 
recent literature. Intuitively, our first result has a running 
time that depends on the distance between $q$ and $q^\pm$: 
it is especially efficient when the input $q$ is either very 
close to or very far from $q^-$ or $q^+$; our second result 
depends on some global notion of distance in the set $S$,
and it is fast when the elements of $S$ are more or less equally 
...
the query and the finger.

In the dynamic setting,
we improve the space requirement in 
a recent result by Bose~\etal [CGTA, 46(2), pp.~181--189].
In particular, we present a 
dynamic data structure for predecessor searching in 
$U$.  Our structure needs $O(\log \log \min\{|q - q^-|, |q - q^+|)$ 
expected time for queries updates, and it requires 
$O(nw)$ bits of space.
As observed by Bose~\etal, the structure can be applied 
for answering approximate nearest
...
recently showed several tight lower bounds that
depend on the word length $w$, the number $n$  of
elements in $S$, and
the number of available storage bits. For example,
in the setting that $w = \Theta(\log n)$ and that
we desire the space requirement to be linear
in the size of $S$, data structures similar to the 
van-Emde-Boas tree (e.g., Willard's $y$-fast tries~\cite{Willard83}) 
with query time $O(\log w)$ are optimal.
...
I the literature, there are several structures for predecessor
searching whose performance guarantees 
depend on the parameters $\Delta_M$, $\Delta_m$, $d(q, S)$, and $D(q, S)$.
In 2003, Demaine~\etal~{\cite{DemaineJoPa04}
presented an \emph{interpolation-search}
structure for dynamic predecessor search. Their structure
requires $O(nw)$ bits of space, and it supports
predecessor queries in $O(\log (\Delta_M/\Delta_m))$ steps.
In 2013, Bose~\etal~\cite{BoseDoDuHoMo13} described
a word-RAM data structure for the \emph{dynamic} predecessor
problem 
that can answer predecessor 
...
In 2015, Bille~\etal~\cite[Section~3]{BilleLaRaSaSaWe15} presented
\emph{interval-biased search trees}. These trees require
$O(nw)$ bits of space, and they support predecessor queries
in $O(w - \log(q^+ - q^-))$ time. Since 
$D(q, S) \leq q^- - q^+ \leq 2D(q, S)$,
this bound can be stated as  $O(w - \log D(q, S))$  in our notation.
In fact, Bille~\etal proved
a slightly stronger result: suppose we have a \emph{finger} $r \in S$
into the data structure,
with $r < q$. Then, we can answer a predecessor query in time
$O(\log(2^w - r) - \log D(q, S))$. Similar finger-based
results where also reported by Andersson and Thorup~\cite{AnderssonTh07},
but they are not quite comparable, since the query time depends
on slightly different parameters (i.e., the number of elements between
$r$ and $q$ that lie in $S$).


Here, we describe data structures for the
static and dynamic predecessor problem
on the word-RAM that provide significant 
improvements over those previous bounds.
...
\begin{enumerate}
  ...
  $O(nw)$ bits of space).
  \item Using $O(nw)$ bits for our structure, we can match the dynamic
  update and search time $O(\log\log d(q, S))$
  of Bose~\etal~\cite{BoseDoDuHoMo13}.
  \item 
  We improve exponentially over the performance of interval-biased search
  trees~\cite[Section~3]{BilleLaRaSaSaWe15}, answering 
  predecessor queries in time
  $O(\log(w - \log D(q, S)))$, with 
  $O(n\log w)$ additional bits of space.
  \item We improve exponentially over structure
  of Demaine~\etal~\cite{DemaineJoPa04}, answering
  predecessor queries in time 
  ...
  exponentially over interval-biased search trees~\cite{BilleLaRaSaSaWe15}.
\end{enumerate}
...
a variant of binary search that bisects the search interval
in a more predictable manner.
The main new observation is that
we can use the additional structure in $S$ in order
to restrict the initial search interval.
This confirms the intuition that fat
...
a \emph{finger} $r \in S$ to locate quickly an extent $e$ 
that is a prefix of $q$ and that has the property 
that $w - |e| \leq \log |q - r|$. The extent
...
$O(\log(w - |e| - \log D(q, S)|) = O(\log(\log|q - r|- \log D(q, S)))$.

\begin{algorithm}
\KwIn{a nonempty string $q \in \{0, 1\}^w$ and a $r \in S$ such that $r < q$}
\KwOut{an index $i$ with $S[i] = q^-$}

$t \gets \max\{\,s \mid r[0, s - 1] + 1 \preceq q\,\}$\;
$e \gets \extent(r[0, t - 1] + 1)$\;
\If{$r[0, t - 1] + 1 \not\preceq e$}{%
  \Return $\rrange(r[0, t - 1])$
...
\tcp{$q$ exits between $r[0, t - 1] + 1$ and $e$} 
$a \gets 0$\;
\tcp{Now $e \prec q$ and $w - |e| \leq \log |q - r|$} 
\While{$a < (w - |e|)/2$}{%
...
a finger $r \in S$, with $r < q$, in time 
$O(\log(\log|q - r|-\log D(q,S)))$, using an 
