...
\[
Q=\bigcup_{\text{node $\alpha$ of $T$}}
\min{}_\preceq\{\,p\in P\mid n_\alpha\preceq p\preceq e_\alpha\,\}
\]
For every node $\alpha$ of $T$, the set $Q$ contains
...
\begin{proof}
First we show correctness. It can be easily seen that at each
step $a$ is either $0$ or the length of an internal extent that is a prefix of
$q$. Moreover, if there is an internal extent of length at least $m$ that is
a prefix of $q$, then $t\neq\bot$, so we if we exit at the first return
instruction, the fat binary search completes correctly. If $t \neq \bot$,
we know that $q[0,t - 1]$ is the name of a node $\alpha$
(because $[a + 1,w - 1])$ is a union of consecutive skip intervals, 
and the smallest power of $2$ in such an interval $[a+1,w - 1]$ is
