...
Naturally, Algorithm~\ref{algo:query-explicit} has
the same properties as Algorithm~\ref{algo:query},
as stated in the following theorem. We omit the proof,
since it is identical to the proof of Theorem~\ref{thm:correctnessfbs}.
\begin{theorem}
\label{thm:correctnessfbs-explicit}
Algorithm~\ref{algo:query-explicit} takes at most $\lceil\log(b-a)\rceil$
iterations
and returns a pointer to $\exit(q)$.
\end{theorem}
...
$w - O(\log d(q, S))$.
Thus, by Jensen's inequality, we will
hit a prefix in $P$ after $O(\log \log \log d(q, S))$
iterations in expectation. After that, by Jensen's
inequality and by Theorem~\ref{thm:correctnessfbs-explicit},
...
\While{$2^{2^i} \leq (w + 1)/2$}{%
  $p \gets (q + r)\bigl[0, w + 1 - 2^{2^i} - 1\bigr]$\;
  $v \leftarrow g(p)$\;
  \If{$v \neq \bot$ }{%
       \If{$e_v \prec q + r$}{%
           \tcp{We found a long extent}
           \Return{$\fbs^-(q + r,|e_v|,w + 1)$}\;
       } \Else {
       \tcp{We exit at $v$}
       \Return{$v$} 
       }
...
\begin{algorithm}
\KwIn{a query string $q \in \{0, 1\}^w$ and a fixed 
random shift $r \in \{0, 1\}^w$}
\KwOut{a pointer to the exit node of $q$ in $T$}

 $a \gets 0$\;
\While{$a<w/2$}{%
  $m \gets \text{least power of $2$ in $[a+1,w]$}$\;
  $v \gets \widehat g((q + r)[0, m-1])$\;
  \If{$v = \bot$}{%
     \Return{$\fbs^-(q + r,a,m)$}
     \tcp{We obtained the longest possible prefix} 
  }
...
\Return{$\fbs^-(q + r, a, w + 1)$}

\caption{Long-distance speedup in the dynamic setting.}
\label{algo:pred-long-explicit}
\end{algorithm}


\subsubsection{Finding the Predecessor Node}
\label{sec:rangeloc}
